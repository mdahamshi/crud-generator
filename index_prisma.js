...
async function createModel(modelNameRaw, fieldArgs = []) {
  ...
  await fs.writeFile(paths.query, await renderTemplate('query_prisma.ejs', data));
  console.log(`✔ Created query file`);

  await fs.writeFile(paths.controller, await renderTemplate('controller_prisma.ejs', data));
